...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // デバッグ用：ルート選択テストボタンを追加
    const routeTestA = document.createElement('button');
    routeTestA.textContent = 'テスト: ルートA';
    routeTestA.style.cssText = `
        position: absolute;
        top: 130px;
        right: 10px;
        z-index: 1000;
    ...
    `;
    routeTestA.onclick = () => {
        console.log('テスト: ルートA選択');
        handleBranchSelection('A');
    };
    document.body.appendChild(routeTestA);

    const routeTestB = document.createElement('button');
    routeTestB.textContent = 'テスト: ルートB';
    routeTestB.style.cssText = `
        position: absolute;
        top: 160px;
        right: 10px;
        z-index: 1000;
    ...
    `;
    routeTestB.onclick = () => {
        console.log('テスト: ルートB選択');
        handleBranchSelection('B');
    };
    document.body.appendChild(routeTestB);
    
    // デバッグ用：現在のシーン画像確認ボタンを追加
    const imageTestButton = document.createElement('button');
    imageTestButton.textContent = '画像確認';
    imageTestButton.style.cssText = `
        position: absolute;
        top: 190px;
        right: 10px;
        z-index: 1000;
        ...
        font-size: 12px;
    `;
    imageTestButton.onclick = () => {
        console.log('--- 画像状態確認 ---');
        console.log('現在のシーン:', currentScene);
        console.log('シーンデータ:', SCENES[currentScene]);
        console.log('現在の背景画像:', SCENE_IMAGE.style.backgroundImage);
        console.log('期待する画像:', SCENES[currentScene]?.image);
    };
    document.body.appendChild(imageTestButton);

    // 各ルートの画像を手動で切り替えるテストボタン
    const testDangerButton = document.createElement('button');
    testDangerButton.textContent = 'Danger画像テスト';
    testDangerButton.style.cssText = `
        position: absolute;
        top: 220px;
        right: 10px;
        z-index: 1000;
        ...
        font-size: 12px;
    `;
    testDangerButton.onclick = () => {
        console.log('Danger画像テスト');
        changeScene('danger_route');
    };
    document.body.appendChild(testDangerButton);

    const testSafeButton = document.createElement('button');
    testSafeButton.textContent = 'Safe画像テスト';
    testSafeButton.style.cssText = `
        position: absolute;
        top: 250px;
        right: 10px;
        z-index: 1000;
        ...
        font-size: 12px;
    `;
    testSafeButton.onclick = () => {
        console.log('Safe画像テスト');
        changeScene('safe_route');
    };
    document.body.appendChild(testSafeButton);
    
    // デバッグ用：ゲーム状態リセットボタンを追加
    ...
    `;
    resetButton.onclick = () => {
        console.log('手動ゲームリセット');
        showStartScreen(); // restartGame()をshowStartScreen()に変更
    };
    ...
    setTimeout(() => {
        ...
        if (startScene && startScene.hotspots && startScene.hotspots.length > 0) {
            const firstHotspot = startScene.hotspots[0];
            console.log('最初のホットスポット:', firstHotspot);
            console.log('移動先:', firstHotspot.nextScene);
            startSceneMove(firstHotspot.nextScene, firstHotspot.timeCost, firstHotspot.event);
        } else {
